...
var TriviaConverter = require('./trivia-converter');
(function (EAtRule) {
    EAtRule[EAtRule["SIMPLE"] = 0] = "SIMPLE";
    EAtRule[EAtRule["RULE_LIST"] = 1] = "RULE_LIST";
    EAtRule[EAtRule["DECLARATION_LIST"] = 2] = "DECLARATION_LIST";
})(exports.EAtRule || (exports.EAtRule = {}));
var EAtRule = exports.EAtRule;
// ==================================================================
// CONVENIENCE METHODS
// ==================================================================
function parse(styleSheetSrc, options) {
...
exports.parse = parse;
function parseRule(ruleSrc, options) {
    var parser = new Parser(ruleSrc, options), t = parser.getCurrentToken(), ret;
    if (!t)
        return null;
...
function parseSelectors(selectorsSrc, options) {
...
function parseSelector(selectorSrc, options) {
...
function parseDeclarations(declarationsSrc, options) {
...
function parseDeclaration(declarationSrc, options) {
    var parser = new Parser(declarationSrc, options), t = parser.getCurrentToken(), token, len, i, ret;
    if (!t)
        return null;
...
function initAtRules() {
    exports.atRules.push({ keyword: 'charset', astClass: AST.AtCharset, type: EAtRule.SIMPLE }, { keyword: 'custom-media', astClass: AST.AtCustomMedia, type: EAtRule.SIMPLE }, { keyword: 'document', astClass: AST.AtDocument, type: EAtRule.RULE_LIST }, { keyword: 'font-face', astClass: AST.AtFontFace, type: EAtRule.DECLARATION_LIST }, { keyword: 'host', astClass: AST.AtHost, type: EAtRule.RULE_LIST }, { keyword: 'import', astClass: AST.AtImport, type: EAtRule.SIMPLE }, { keyword: 'keyframes', astClass: AST.AtKeyframes, type: EAtRule.RULE_LIST }, { keyword: 'media', astClass: AST.AtMedia, type: EAtRule.RULE_LIST }, { keyword: 'namespace', astClass: AST.AtNamespace, type: EAtRule.SIMPLE }, { keyword: 'page', astClass: AST.AtPage, type: EAtRule.DECLARATION_LIST }, { keyword: 'supports', astClass: AST.AtSupports, type: EAtRule.RULE_LIST });
}
function getLastToken(n) {
    var children, len;
...
var Parser = (function () {
    ...
    Parser.prototype.parseStyleSheet = function () {
        var cdo = null, cdc = null, ruleList;
        ...
        return new AST.StyleSheet(ruleList || new AST.RuleList([], null, null), cdo, cdc);
    };
    /**
     ...
     */
    Parser.prototype.parseRuleBlock = function () {
     ...
     * @returns {AST.RuleList}
     */
    Parser.prototype.parseRuleList = function (isBlock) {
        var rule, rules = [], lbrace = null, rbrace = null, token;
        if (isBlock) {
            // consume '{'
     ...
     * @returns {AST.Rule}
     */
    Parser.prototype.parseQualifiedRule = function () {
        var selector, selectors = [], selectorList = null, declarationList = null, token;
        // Repeatedly consume the next input token:
        for (;;) {
            token = this._currentToken.token;
            if (token === Tokenizer.EToken.EOF) {
    ...
    Parser.prototype.parseAtRule = function () {
        var atKeyword = this._currentToken, spec = this.getAtRuleSpec(atKeyword), preludeValues, prelude = undefined, blockOrSemicolon, t, token;
     ...
     */
    Parser.prototype.parseDeclarationList = function () {
        var lbrace, rbrace = null, token, declaration, disabledDeclarations, declarations = [], prevIsDeclaration = false;
        // consume '{'
        this.expect(Tokenizer.EToken.LBRACE);
        lbrace = this._currentToken;
     ...
     */
    Parser.prototype.parseDeclaration = function (throwErrors, omitSemicolon) {
        if (throwErrors === void 0) { throwErrors = true; }
        var t = this._currentToken, nameValues, name, colon, value, semicolon, lcomment, rcomment;
     ...
     * @returns {*}
     */
    Parser.prototype.parseTrailingTokensForDisabledDeclarations = function (token) {
        var declarations, declaration, originalTrailingTrivia, lastTrailingTrivia, len, i, t, tokens, lastToken = token;
        var updateRange = function (t, defaultRange) {
            var trivia = t.trailingTrivia, len = trivia.length, range, endLine, endColumn, node = t;
            if (len === 0) {
                if (!defaultRange)
                    return;
     ...
     * @returns {*}
     */
    Parser.prototype.parseDisabledDeclaration = function (token, throwErrors) {
        if (throwErrors === void 0) { throwErrors = true; }
        var declaration;
        if (token.token !== Tokenizer.EToken.COMMENT)
            return null;
     ...
     */
    Parser.prototype.parseDeclarationValue = function () {
     ...
     */
    Parser.prototype.parseSelectorList = function () {
        var selector, selectors = [], t, token;
        for (;;) {
            t = this._currentToken;
            token = t.token;
    ...
    Parser.prototype.parseSelector = function () {
        var t, token, value, values = [], len, ident, namespace, pipe, colon1, colon2, separator;
        try {
                ...
                if (token === Tokenizer.EToken.COMMA) {
                    ...
                    if (value) {
                        if (token === Tokenizer.EToken.LBRACKET) {
                            value = new AST.AttributeSelector(value, namespace, pipe);
                            namespace = null;
                            pipe = null;
                        }
                        values.push(value);
        ...
        return ((values && values.length > 0) || separator) ? new AST.Selector(values, separator) : null;
    };
    /**
     ...
     */
    Parser.prototype.parseComponentValueList = function () {
        var endTokens = [];
        for (var _i = 0; _i < arguments.length; _i++) {
            endTokens[_i - 0] = arguments[_i];
        }
        var value, values = [], t, token, prevToken;
        try {
            for (t = this._currentToken;;) {
                token = t.token;
    ...
    Parser.prototype.parseBlock = function () {
        var startToken = this._currentToken, token = startToken.token, endingToken = undefined, t, value, values = [];
        if (token === Tokenizer.EToken.LBRACE)
    ...
    Parser.prototype.parseFunction = function () {
        var t, token, name, arg, separator, args = [];
        ...
        for (t = this._currentToken;;) {
            ...
            if (token === Tokenizer.EToken.EOF)
                ...
                arg = this.parseComponentValueList(Tokenizer.EToken.COMMA, Tokenizer.EToken.RPAREN, 
    ...
    Parser.prototype.expect = function () {
        ...
        var tokens = [], parsedNodes = [], len = args.length, i, j, l, arg, a;
        // parse the arguments
        for (i = 0; i < len; i++) {
            arg = args[i];
     ...
     * @param nodes
     */
    Parser.prototype.rethrow = function (e, nodes) {
     ...
     * @param nextTokens
     */
    Parser.prototype.cleanup = function (e, endTokens, nextTokens) {
        var t, token, errorTokens = [], len, node, tokens, lenTokens, i, j;
     ...
     * @returns {*}
     */
    Parser.prototype.getAtRuleSpec = function (atKeyword) {
        var value = atKeyword.value, isPrefixed = value[0] === '-', len = exports.atRules.length, i, spec, keyword;
        for (i = 0; i < len; i++) {
            spec = exports.atRules[i];
